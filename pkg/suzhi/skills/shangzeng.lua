local shangzeng = fk.CreateSkill {
  name = "shangzeng",
}
...
shangzeng:addEffect("active",{
  prompt = "#shangzeng-active",
  anim_type = "drawcard",
  can_use = function(self, player)
    return player:hasSkill(self) and player:usedSkillTimes(shangzeng.name, Player.HistoryRound) == 0 
...
shangzeng:addEffect("targetmod", {
  bypass_times = function(self, player, skill, scope, card, to)
    return card and player:hasSkill(shangzeng.name) and player:getMark("shangzeng-turn") > 0
  end,
...
shangzeng:addEffect(fk.AfterCardUseDeclared, {
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(shangzeng.name) and player:getMark("shangzeng-turn") > 0
  end,
  on_cost = Util.TrueFunc,
  on_use = function(self, event, target, player, data)
    data:changeCard(data.card.name, Card.NoSuit, data.card.number, shangzeng.name)
